"""
UOFast MCP Server — Admin Setup
=================================
Multi-step configuration at /admin/setup.
Requires an active admin session (login at /admin first).

Steps:
  1. /admin/setup/welcome      — prerequisites check
  2. /admin/setup/security     — JWT secret + admin password
  3. /admin/setup/connection   — UniData connection settings + live test
  4. /admin/setup/complete     — write config, init DB, show .env
  5. /admin/setup/client-setup — Claude Desktop / VSCode / CLI configs
"""
...
def _build_env_content(jwt_secret: str, admin_password: str) -> str:
    ...
    return (
        ...
        f"# Generated by Admin Setup on {now}\n"
...
async def _run_setup(request: Request) -> dict:
    ...
    try:
        ...
        result["errors"].append(f"Failed to create/update admin user: {exc}")
    ...
    # Reload connection config into the running server immediately.
    try:
...
async def setup_root(request: Request):
    ...
    return RedirectResponse("/admin/setup/welcome", status_code=302)
...
async def security_post(request: Request):
    ...
    return RedirectResponse("/admin/setup/connection", status_code=303)
...
async def connection_get(request: Request):
    ...
    if not request.session.get("setup_security_done"):
        return RedirectResponse("/admin/setup/security", status_code=302)
...
async def connection_post(request: Request):
    ...
    if form.get("skip"):
        ...
        return RedirectResponse("/admin/setup/complete", status_code=303)
...
async def client_setup_get(request: Request):
    ...
    if not request.session.get("setup_complete"):
        return RedirectResponse("/admin/setup/complete", status_code=302)
